...
with st.sidebar:
    ...
    if not eng_df.empty:
        ...
        min_market_cap = st.number_input("Min Market Cap ($M)", min_value=0.0, value=0.0, step=100.0)
        min_yield = st.number_input("Min Grossed-Up Yield (%)", min_value=0.0, value=0.0, step=0.5)
        show_only_franked = st.checkbox("Franked Dividends Only", value=False)

        # Apply filters
        filtered_eng = eng_df.copy()
        if min_market_cap > 0:
            filtered_eng = filtered_eng[filtered_eng["market_cap"] >= min_market_cap * 1e6]
        if min_yield > 0:
            filtered_eng = filtered_eng[filtered_eng["grossed_up_yield"] >= min_yield / 100]
        if show_only_franked:
